...
class Agent:
    def __init__(self, cfg):
        ...
        self.states = ['box', 'mercenaries', 'team_lock', 'travel', 'boss_list', 'team_list', 'map_not_ready',
                       ...
                       'final_reward', 'final_reward2', 'final_reward3', 'final_confirm', 'close', 'ok', 'done',
                       'member_not_ready2', 'member_not_ready3']
    ...
    def state_handler(self, state, tic, text):
        ...
        self.states = ['box', 'mercenaries', 'team_lock', 'travel', 'boss_list', 'team_list', 'map_not_ready',
                  'goto', 'show', 'teleport', 'start_game', 'member_not_ready', 'not_ready_dots', 'battle_ready',
                  'treasure_list', 'treasure_replace', 'destroy', 'blue_portal', 'boom', 'visitor_list',
                  'final_reward', 'final_reward2','final_reward3', 'final_confirm', 'ok', 'close', 'done', 'member_not_ready2','member_not_ready3']
        '''
        if success:
            if state != text:
                state = text
            ...
            if state in ['member_not_ready', 'member_not_ready2', 'member_not_ready3']:
                logger.info(f'find {state}, try to click')
    ...
    def run_pve(self):
        ...
        while True:
            ...
            if time.time() - tic > self.basic.longest_waiting:
                ...
                if state == 'not_ready_dots' or state == 'member_not_ready' or state == 'member_not_ready2' or state == 'member_not_ready3':
